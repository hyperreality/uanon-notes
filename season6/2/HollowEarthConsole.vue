    ...
    <div class="console-inner">
      ...
      <div class="uwrapper">
        ...
        <div class="cli-input" v-if="!dollhouse.started">
          <span class="cyan">{{ uni }}</span
          >@{{ r }}:<span class="jaundice"
            >~/{{ r == "summer" ? i + 1 : i }}</span
          ><span> $ </span>
          <input
            type="search"
        ...
        <div class="cli-input" v-if="dollhouse.started">
          <span class="cyan" v-if="dollhouse.started == true">{{ uni }}</span
          >@{{ r }}:<span class="jaundice">~/HOLLOWEARTH</span><span> $ </span>
          <input
            type="search"
            id="cli"
            class="input"
    ...
    this.$nextTick(function () {
      if (this.iu) {
        setTimeout(() => {
          document
            .getElementById("cli")
            .setSelectionRange(this.input.length, this.input.length);
        }, 1);
        this.iu = false;
      }
    });
  ...
  methods: {
    ...
    dollhouseCp: function (cmd) {
      ...
      this.log.push(
        '<span class="cyan">' +
          ...
          "</span>@" +
          this.r +
          ':<span class="jaundice">~/HOLLOWEARTH/</span><span> $ </span> ' +
          cmd
      );
    ...
    dollhouseMove: function (direction) {
      ...
      if (room.options.indexOf(direction) == -1) {
        // Direction invalid
        let rand = this.getRandomInt(
          0,
          DollhouseCore.globals.walk.failure.length - 1
        );
    ...
    dollhouseDig: function (cmd) {
      ...
      let parts =
        cmd.indexOf("DIG") > -1
          ? cmd.replaceAll(" ", "").split("DIG")
          : cmd.replaceAll(" ", "").split("D");
      ...
      if (
        ...
        let rand = this.getRandomInt(
          0,
          DollhouseCore.globals.dig.failure.length - 1
        );
        ...
        dimensions[0] > room.size.w ||
        dimensions[0] < 1 ||
        dimensions[1] > room.size.l ||
        dimensions[1] < 1
      ) {
        let rand = this.getRandomInt(
          0,
      ...
      try {
        ...
        let rand = this.getRandomInt(
          0,
          DollhouseCore.globals.dig.success.length - 1
        );
    ...
    dollhouseFeel: function (cmd) {
      ...
      let parts =
        cmd.indexOf("FEEL") > -1
          ? cmd.replaceAll(" ", "").split("FEEL")
          : cmd.replaceAll(" ", "").split("F");
      let object = parts.pop();
      ...
      if (!room["keys"]) {
        let rand = this.getRandomInt(
          0,
          DollhouseCore.globals.feel.failure.length - 1
        );
      ...
      try {
        ...
        let rand = this.getRandomInt(
          0,
          DollhouseCore.globals.feel.success.length - 1
        );
    ...
    dollhouseGameover: function (type = 0) {
      ...
      this.log.push(
        this.dollhouseFmt(DollhouseCore.globals.gameover.failure.msg)
      );
    ...
    dollhouseVictory: function () {
      ...
      this.log.push(
        this.dollhouseFmt(DollhouseCore.globals.gameover.success.msg)
      );
    ...
    dollhouseDizzy: function () {
      ...
      if (roll) {
        ...
        let rand = this.getRandomInt(
          0,
          DollhouseCore.globals.feel.madness.length - 1
        );
    ...
    dollhouseFatigue: function () {
      ...
      if (roll) {
        ...
        let rand = this.getRandomInt(
          0,
          DollhouseCore.globals.dig.fatigue.length - 1
        );
    ...
    dollhouseUtter: function (cmd) {
      let parts =
        cmd.indexOf("UTTER") > -1
          ? cmd.replaceAll(" ", "").split("UTTER")
          : cmd.replaceAll(" ", "").split("U");
      let object = parts.pop();
      // console.log('dollhouseUtter', cmd, object);
      if (this.dollhouse.level >= 9) {
        let rand = this.getRandomInt(
          0,
          DollhouseCore.globals.incantation.failure - 1
        );
      ...
      try {
        ...
        if (!value) {
          let rand = this.getRandomInt(
            0,
            DollhouseCore.globals.incantation.failure.length - 1
          );
          let failedMsg = DollhouseCore.globals.incantation.failure[rand];
          this.log.push(failedMsg);
          return;
      ...
      let rand = this.getRandomInt(
        0,
        DollhouseCore.globals.incantation.success.length - 1
      );
    ...
    dollhouseOpen: function (cmd) {
      ...
      try {
        ...
        for (let i = 0; i < this.dollhouse.inventory.chests.length; i++) {
          if (this.dollhouse.inventory.chests[i].name == chestName)
            this.dollhouse.inventory.chests[i].engraving = legend;
        }
        this.log.push(DollhouseCore.globals.open.chest.success);
        this.log.push(
          '<span class="cyan">You uncover the following message inside</span>: <a href="https://' +
            legend +
            '" target="_blank">' +
            legend +
            "</a>"
    ...
    dollhouseLook: function (cmd) {
      let parts =
        cmd.indexOf("LOOK") > -1
          ? cmd.replaceAll(" ", "").split("LOOK")
          : cmd.replaceAll(" ", "").split("L");
      let object = parts.pop();
      let room = DollhouseCore.rooms[this.dollhouse.room];
    ...
    dollhouseInventory: function (type = 0) {
      ...
      if (!type) {
        ...
        this.dollhouse.inventory.keys.forEach((key) => {
          ...
          msgs.push(
            i +
              ...
              key.name +
              ', <span class="cyan">Engraving</span>: ' +
              key.engraving
          );
        });
        ...
        if (!n) {
          this.log.push(
            '<span class="blood" style="font-style:italic;">You don\'t have any keys</span>'
          );
          return;
        }
        // Chests
        ...
        if (!n) {
          this.log.push(
            '<span class="blood" style="font-style:italic;">You don\'t have any chests</span>'
          );
          return;
        }
      }
      if (n > 0) {
        this.log.push(
          '<span class="cyan">You find the following ' +
            item +
            " in your inventory: </span>"
        );
        msgs.forEach((msg) => {
          this.log.push(msg);
        });
    ...
    dollhouseSize: function () {
      let room = DollhouseCore.rooms[this.dollhouse.room];
      this.log.push(
        '<span class="cyan">The size of this room is</span>: ' +
          room.size.w +
          " x " +
          room.size.l
      );
    ...
    dollhouseRoom: function () {
      ...
      if (text.secret) {
        ...
        if (!proof) {
          this.log.push(
            '<span class="blood" style="font-style:italic;">' +
              DollhouseCore.globals.gate.msg +
              "</span>"
          );
      ...
      // Display room status
      this.log.push(
        '<span class="jaundice" style="font-style:italic;">' +
          this.dollhouseFmt(text.value) +
          "</span>"
      );
      this.log.push(
        '<span class="cyan">You are located in</span>: ' + room.title
      );
      this.log.push(
        '<span class="cyan">There are ' +
          room.objects.length +
          " objects here:</span> " +
          String(room.objects)
      );
    ...
    cp: function (cmd) {
      ...
      if (typeof cmd !== "string") {
        ...
        return (
          '<pre class="log cyan">' + JSON.stringify(st, null, 2) + "</pre>"
        );
        ...
        let f =
          Config.DEFAULT_STORAGE_BASE +
          "/" +
          id +
          "/" +
          Config.DEFAULT_STORAGE_DOWNLOAD;
        this.download(f);
        ...
        if (
          !can.go &&
          location.href.indexOf("rites-of-spring") == -1 &&
          location.href.indexOf("the-land-vomits-ghosts") == -1 &&
          location.href.indexOf("the-same-old-story") == -1
        ) {
    ...
    ig: function () {
      ...
      if (this.p.title) {
        if (typeof this.p.title == "object") {
          ig.push(
            ' <span class="cyan-bg">Title:</span>                                     ' +
              this.p.title.value
          );
          ig.push(
            ' <span class="cyan-bg">Title<span class="jaundice">->format</span>:</span>           ' +
              this.p.title.format
          );
        } else {
          ig.push(
      ...
      if (this.p.description) {
        if (typeof this.p.description == "object") {
          ig.push(
            ' <span class="cyan-bg">Description:</span>                               ' +
              this.p.description.value
          );
          ig.push(
            ' <span class="cyan-bg">Description<span class="jaundice">->format</span>:</span>     ' +
              this.p.description.format
          );
        } else {
          ig.push(
            ' <span class="cyan-bg">Description:</span>                               ' +
      ...
      if (this.p.secret) {
        ig.push(
          ' <span class="cyan-bg">Public Key:</span>                                  ' +
            this.p.secret
        );
      }
      if (this.p.operation) {
        let o =
          this.p.operation.mainnet == ""
            ? this.p.operation.testnet
            : this.p.operation.mainnet;
        ig.push(
          ' <span class="cyan-bg">Operation:</span>                                   ' +
            o
        );
      }
      if (this.p.category) {
        ig.push(
          ' <span class="cyan-bg">Category:</span>                                  ' +
            this.p.category
        );
      }
      if (this.p.tags) {
        ig.push(
          ' <span class="cyan-bg">Tags:</span>                                  ' +
            this.p.tags.join(", ")
        );
      }
      if (this.p.points) {
        ig.push(
          ' <span class="cyan-bg">Points:</span>                                  ' +
            this.p.points
        );
      }
      if (this.p.code) {
      ...
      if (this.p.payload) {
        if (this.p.payload.value) {
          if (Array.isArray(this.p.payload.value)) {
            ...
            if (typeof this.p.payload.value == "object") {
              ...
              ig.push(
                ' <span class="cyan-bg">Payload:</span>                                 ' +
                  this.p.payload.value
              );
            }
          }
        }
        if (this.p.payload.format) {
          if (this.r == "spring" && this.i == 6) {
            let arr = [
              '<span class="cyan-bg">Format:</span> Each person, 1st input - ' +
                this.p.payload.format.person1.input1,
              '<span class="cyan-bg">Format:</span> Each person, 2nd input - ' +
                this.p.payload.format.person1.input2,
              '<span class="cyan-bg">Format:</span> Each person, 3rd input - ' +
                this.p.payload.format.person1.input3,
              '<span class="cyan-bg">Format:</span> Each person, 4th input - ' +
                this.p.payload.format.person1.input4,
              '<span class="cyan-bg">Format:</span> Each person, 5th input - ' +
                this.p.payload.format.person1.input5,
            ];
            ...
            if (Array.isArray(this.p.payload.format)) {
              for (let i = 0; i < this.p.payload.format.length; i++) {
                ig.push(
                  '<span class="cyan-bg">Format ' +
                    (i + 1) +
                    ":</span>                   " +
                    this.p.payload.format[i]
                );
              }
            } else {
              if (!this.p.format) {
                ig.push(
                  '<span class="cyan-bg">Format:</span>                               ' +
                    this.p.payload.format
                );
              }
            }
          }
      ...
      if (this.cf) {
        if (this.r == "summer" && this.i == 6) {
          ...
          ig.push(
            '<span class="cyan-bg">Email Reply:<span class="jaundice">->payload</span>:</span>                ' +
              p
          );
          ig.push(
            '<span class="cyan-bg">Email Reply:<span class="jaundice">->format</span>:</span>                 ' +
              f
          );
        }
      }
      if (this.p.format) {
        if (
          Array.isArray(this.p.format) &&
          !Array.isArray(this.p.payload["format"])
        ) {
      ...
      if (this.r == "spring+" && this.p.morals) {
        if (Array.isArray(this.p.morals)) {
          if (this.p.morals.length) {
            ig.push(
              ' <span class="cyan-bg">Fear<span class="jaundice">->format</span>:</span>                      ' +
                this.p.morals[0].format
            );
          }
        }
      }
      if (this.p.warning) {
        ig.push(
          ' <span class="cyan-bg">Warning:</span>                                      ' +
            this.p.warning
        );
      }
      if (this.p.hint) {
        ...
        if (this.p.files) {
          if (Array.isArray(this.p.files)) {
            if (this.p.files.length > 10) {
              ig.push(
                ' <span class="cyan-bg">Attachments:</span><span style="font-style:italic;"> ' +
                  this.p.files.length +
                  " files</span>"
              );
            } else {
              for (let i = 0; i < this.p.files.length; i++) {
                if (typeof this.p.files[i] == "object") {
                  ...
                  if (!link || !name) {
                    ig.push(
                      ' <span class="cyan-bg">Attachment ' +
                        (i + 1) +
                        ":</span>               " +
                        String(this.p.files[i])
                    );
                  } else {
                    ig.push(
                      '<span class="cyan-bg">Attachment ' +
            ...
            ig.push(
              ' <span class="cyan-bg">Attachments:</span>                              ' +
                String(this.p.files)
            );
          }
        }
     ...
     */
    make: function (a) {
      let p = this.h.g(JSON.stringify([a]), Config.DEFAULT_DEPTH),
        s = this.s,
        can = this;
      p = p.substring(2);
...
.helper-bar:hover {
  ...
  text-shadow: 0 0 20px #eee, 0 0 30px #eee, 0 0 40px #ff7070, 0 0 50px #ff4da6,
    0 0 60px #ff4da6, 0 0 70px #ff4da6, 0 0 80px #ff7070;
}
</style>
<style>
.wrapme {
