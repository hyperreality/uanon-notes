...
def brute(secret, wordlist, length=1, capitalize=False, upper=False, lower=False, depth=2):
    wordlist1 = [w.strip() for w in open('i', 'r').readlines()]
    wordlist2 = [w.strip() for w in open('on', 'r').readlines()]
...
brute(spring7, wordlist, length=2, depth=2, upper=False, lower=False)


# sort -nr -k 2 spell_dict_with_freq.dic | awk '{ print $1 }' > words_freq.greek
# grep I words_freq.english | sed -e s'/[A-Za-z]\{9,\}//g' | sort | uniq > i
# grep ON words_freq.english | sed -e s'/[A-Za-z]\{9,\}//g' | sort | uniq > on
